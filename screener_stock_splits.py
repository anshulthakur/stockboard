...
SPLIT_URL = f"{BASE_URL}/actions/split/"
...
def fetch_splits_page(page_url):
    ...
    for row in table_rows:
        ...
        stock_link = row.select_one('.field-company_display a')
        stock_data['stock'] = stock_link.text.strip()
        stock_data['details_url'] = f"https://www.screener.in{stock_link['href']}"
        
        # Extract other details
        stock_data['ex-date'] = columns[0].text.strip()
        stock_data['old-face-value'] = columns[1].text.strip()
        stock_data['new-face-value'] = columns[2].text.strip()
        stock_data['nse'] = None
        stock_data['bse'] = None
        try:
...
def main():
    ...
    max_pages = get_max_pages(SPLIT_URL)
    
    # Fetch splits from each page
    for page_num in range(1, max_pages + 1):
        time.sleep(1)
        page_url = f"{SPLIT_URL}?p={page_num}"
        print(f"Fetching page {page_num}...")
        splits = fetch_splits_page(page_url)
        all_splits.extend(splits)

    # Output the data as JSON
    with open('stock_splits.json', 'w', encoding='utf-8') as f:
        json.dump(all_splits, f, ensure_ascii=False, indent=4)

    print(f"Scraping completed. Total split entries: {len(all_splits)}")
    print("Data saved to stock_splits.json")
